// Code generated by goa v3.3.1, DO NOT EDIT.
//
// play client
//
// Command:
// $ goa gen github.com/ebalkanski/goa/design

package play

import (
	"context"

	goa "goa.design/goa/v3/pkg"
)

// Client is the "play" service client.
type Client struct {
	AddEndpoint  goa.Endpoint
	RateEndpoint goa.Endpoint
}

// NewClient initializes a "play" service client given the endpoints.
func NewClient(add, rate goa.Endpoint) *Client {
	return &Client{
		AddEndpoint:  add,
		RateEndpoint: rate,
	}
}

// Add calls the "add" endpoint of the "play" service.
func (c *Client) Add(ctx context.Context, p *AddPayload) (res int, err error) {
	var ires interface{}
	ires, err = c.AddEndpoint(ctx, p)
	if err != nil {
		return
	}
	return ires.(int), nil
}

// Rate calls the "rate" endpoint of the "play" service.
func (c *Client) Rate(ctx context.Context, p *RatePayload) (err error) {
	_, err = c.RateEndpoint(ctx, p)
	return
}
